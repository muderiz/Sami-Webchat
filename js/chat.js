...
function initBroadcast() {
	if (typeof BroadcaseChannel !== "undefined" ) {
...
var Chat = {
	...
	client: null, chat_box: null, login_box: null, notif_box: null, webview: false, mobile: null, isconnecting: false, guestName: "", triggerMenu: "", menu: false,
	...
	initLogin: function () {
		if (Chat.guestName && localStorage.getItem("uid") === null) {
			...
			if (localStorage.getItem("uid") !== null) {
				Chat.access_token = localStorage.getItem("at");
	...
	validatePhone: function (phone, element) {
		if (/^\+?([0-9]{2})\)?[-. ]?([0-9]{8,12})$/.test(phone)) {
			...
			element.text('Please enter minimum 10-12 digit');
		}
	},
	...
	connectServer: function () {
		...
		console.log("cookie on connect server:  " + cookieToken);
		if (Chat.webview || Chat.guestName || (cookieToken !== null && cookieToken !== '') || this.validateName(Chat.name, $("#warning-name")) && this.validateEmail(Chat.email, $("#warning-email")) && this.validatePhone(Chat.phone, $("#warning-phone"))) {
			...
			if (rat !== null && rat.responseJSON !== undefined && rat.responseJSON.access_token !== undefined && rat.responseJSON.refresh_token !== undefined) {
				...
				if (!Chat.client || !Chat.client.connected) {
					...
					Chat.client.connect({ accessToken: Chat.access_token, name: Chat.name, email: Chat.email, phone: Chat.phone, uid: Chat.uid, token: cookieToken, captcha: g_recaptcha }, function (frame) {
						...
						if (Chat.name !== null && Chat.email !== null && Chat.phone !== null && Chat.uid !== null) {
							clientToken = fuse(Chat.name + Chat.email + Chat.phone + Chat.uid);
						} else {
							if (cookieToken !== null) {
	...
	subscribeFeed: function () {
		if (!Chat.webview && localStorage.getItem("messages") !== null) {
			Chat.buildChat(Chat.chat_box, true);
		}
		if (Chat.session_id !== null) {
	...
	buildLogin: function (element) {
		if (!Chat.guestName) {
			...
			if (element.childNodes.length === 0) {
				...
				chat_close.innerHTML = '<button type=\"submit\" class=\"message-close\">_</button>';
	...
	buildChat: function (element, skip) {
		if (element.childNodes.length === 0) {
			...
			if (!Chat.webview && localStorage.getItem("messages") !== null) {
				$(localStorage.getItem("messages")).appendTo($('.messages-content'));
				rebuildButton();
			}
		...
		if (document.getElementById("messages-content") !== null) {
			if (document.getElementById("messages-content").innerHTML === "") {
				if (!skip) {
					setTimeout(function () {
						var welcome = {};
						if (Chat.welcome_message) {
...
function incomingMessageFeed(payload) {
	...
	if (payload.attUrl) {
		...
		message = urlify(message);
		if (agentName !== "") {
			$('<div class="message new"><figure class="avatar"><img src=\"' + avatar + '\"/></figure>' + message + '<br/><span style=\"color:#2ECC71;float:right;font-size:10px;margin-top:3px;\">' + agentName + '</span></div>').appendTo($('.messages-content')).addClass('new');
		} else {
...
function outgoingQuickReply(message, label) {
	...
	if (Chat.client !== null) {
		...
		$('<div class="message message-failed">' + label + '</div>').appendTo($('.messages-content')).addClass('new');
		setDate(true);
	}
	$('.message-input').val(null);
	removeQuickReply();
...
function parseButton(message) {
	...
	buttons.forEach(function (button) {
		if (typeof button.pictureLink !== "undefined" && !isValidUrl(button.pictureLink)) {
			var picture = JSON.parse(button.pictureLink);
...
function renderButton(buttons) {
	...
	buttons.forEach(function (button) {
		...
		var buttonTitle = '';
		if (typeof button.pictureLink !== "undefined") {
			buttonTitle = '<div class="dolphin-btn-card__content"><div class="dolphin-btn-card__title truncate-160" title="' + escapeHtml(button.title) + '">' + escapeHtml(button.title) + '</div>';
		} else {
			buttonTitle = '<div class="dolphin-btn-card__content" style="height:150px"><div class="dolphin-btn-card__title truncate-160" title="' + escapeHtml(button.title) + '">' + escapeHtml(button.title) + '</div>';
		}

		var subTitle = button.subTitle;
		if (subTitle.length > 70) {
			subTitle = subTitle.substr(0, 80) + "...";
		}
		
		var buttonSubtitle = '<p class="dolphin-btn-card__text truncate-160" title="' + escapeHtml(button.subTitle) + '">' + escapeHtml(button.subTitle) + '</p>';
		var buttonActions = '';
		for (var i = 0; i < button.buttonValues.length; i++) {
			var buttonAction = button.buttonValues[i];
			if (!isValidUrl(buttonAction.value.trim())) {
				buttonActions = buttonActions + '<a href="#" class="dolphin-btn" role="button" onclick="outgoingQuickReply(\'' + buttonAction.value.trim() + '\',\'' + buttonAction.name.trim() + '\');return false;"><span>' + escapeHtml(buttonAction.name) + '</span></a>';
			} else {
				if (buttonAction.value.trim().indexOf("form.client") !== -1) {
					var value = buttonAction.value.replace("form.client", "form.client.mini");
					buttonActions = buttonActions + '<span class="dolphin-btn" onclick="viewForm(\'' + value + '\')" style="cursor:pointer;"><span>' + escapeHtml(buttonAction.name) + '</span></span>';
				} else if (buttonAction.value.trim().indexOf("payment.client") !== -1) {
					buttonActions = buttonActions + '<span class="dolphin-btn" onclick="viewPayment(\'' + buttonAction.value + '\')" style="cursor:pointer;"><span>' + escapeHtml(buttonAction.name) + '</span></span>';
				} else {
					buttonActions = buttonActions + '<a target="_blank" href="' + buttonAction.value + '" class="dolphin-btn"><span>' + escapeHtml(buttonAction.name) + '</span></a>';
				}
			}
		}
		var buttonEnd = ""; 
	...
	sleep(250);
	$('<div class="message new dolphin-contain-buttons"><figure class="avatar"></figure>' + buttonHtmls + '</div>').appendTo($('.messages-content')).addClass('new');
	if (buttons.length > 1) {
	...
	replyArray.forEach(function (replyText) {
		if (!replyText.toLowerCase().startsWith("title=")) {
			...
			if (replyValues.length > 1) {
				action.label = escapeHtml(replyValues[0]);
				action.text = escapeHtml(replyValues[1]);
				replyOptions.push(action);
			} else {
				action.label = escapeHtml(replyValues[0]);
				action.text = escapeHtml(replyValues[0]);
				replyOptions.push(action);
			}
		} else {
 ...
 * @returns
 */
function buildQuickReply(message) {
...
function viewFormInChat(url) {
	...
	$('<div class="message new dolphin-contain-buttons"><iframe id="' + frameId + '" src="' + value + '" frameBorder="0" width="270"/></div>').appendTo($('.messages-content')).addClass('new');
...
function uploadFile() {
	...
	var attSize = (file.size / 1024 / 1024).toFixed(4);
	if (attSize <= 2) {
		...
		incomingMessage({ 'body': { 'message': 'Attachment maximum filesize is 2MB' } });
	}
}
...
/**
Handle user input for ignoring XSS
**/
var mapObj = {
   '&':"&amp;",
   '<':"&lt;",
